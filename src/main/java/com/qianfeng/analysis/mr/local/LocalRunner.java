...
import com.qianfeng.analysis.mr.au.ActiveUserMapper;
import com.qianfeng.analysis.mr.au.ActiveUserReducer;
...
public class LocalRunner implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "LOCAL_ACTIVE_USER TO MYSQL");
        job.setJarByClass(LocalRunner.class);

        //设置map相关参数
        job.setMapperClass(LocalMapper.class);
        job.setMapOutputKeyClass(StatsUserLcDimension.class);
        job.setMapOutputValueClass(IdOutputValue.class);


        //设置reduce相关参数
        //设置reduce端的输出格式类
        job.setReducerClass(LocalReduce.class);
        job.setOutputKeyClass(StatsUserLcDimension.class);
